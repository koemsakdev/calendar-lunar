...
export class CalendarStoreService {
  ...
  readonly initDate = computed(() =>
    moment({ y: this.currentYear(), M: this.currentMonth() - 1, D: 1 }).locale('en')
  );
  ...
  initDay(): void {
    ...
    if (date.clone().month() + 1 === 4) {
      this.attributes.update((a) => [...a, ...this.getKhmerNewYearAttrs(date.clone().year())]);
    }
    ...
    const monthEvents = this.publicHolidaysService.events
      ...
      .map((element) => ({
        key: 'events' + moment({ y: date.clone().year(), M: date.clone().month(), D: element.start_date.day })
        ...
        dates: moment({ y: date.clone().year(), M: date.clone().month(), D: element.start_date.day })
          .locale('en').format('YYYY-MM-DD'),
      }));
    ...
    const lastDayIndex = moment(date.format('YYYY-MM-') + currentDaysInMonth).day();
    ...
    for (let x = firstDayIndex; x > 0; x--) {
      ...
      const prevDate = moment(date.clone().subtract(1, 'months').format('YYYY-MM-') + day, 'YYYY-MM-D');
      const khResult = this.kh.fromMoment(prevDate);
      this.applyTraditionalEvents(prevDate, khResult);
      newDays.push({
        prevMonth: true, nextMonth: false, day,
        date: prevDate, khResult,
        attributes: this.attributes().filter((item) => prevDate.isSame(item.dates)),
      });
    }
    ...
    for (let i = 1; i <= currentDaysInMonth; i++) {
      ...
      newDays.push({
        prevMonth: false, nextMonth: false, day: i,
        date: currentDate, khResult,
        attributes: this.attributes().filter((item) => currentDate.isSame(item.dates)),
      });
    ...
    for (let y = 1; y <= nextDaysCount; y++) {
      const nextDate = moment(date.clone().add(1, 'months').format('YYYY-MM-') + y, 'YYYY-MM-D');
      const khResult = this.kh.fromMoment(nextDate);
      this.applyTraditionalEvents(nextDate, khResult);
      newDays.push({
        prevMonth: false, nextMonth: true, day: y,
        date: nextDate, khResult,
        attributes: this.attributes().filter((item) => nextDate.isSame(item.dates)),
      });
    }
    ...
    this.attributes.update((attrs) =>
      [...attrs].sort((a, b) => new Date(a.dates).getTime() - new Date(b.dates).getTime())
    );
  }

  private applyTraditionalEvents(date: moment.Moment, khResult: KhmerDateResult): void {
    ...
    const matchingEvents = this.publicHolidaysService.traditionalEvents.filter(
      (item) => item.start_date.day === dn && item.start_date.month === monthName
    );
    matchingEvents.forEach((element) => {
      this.attributes.update((attrs) => [...attrs, {
        key: 'traditional_events' + date.toString(),
        customData: { title: element.summary, description: element.description, class: 'bg-red-600 text-white' },
  ...
  getBuddhistHolyDay(dn: string, date: moment.Moment): boolean {
    ...
    if (dn === '១៤រោច' && this.kh.toKhDateDN(date.clone().add(1, 'days')) !== '១៥រោច') return true;
    return false;
  }
  ...
  private getKhmerNewYearAttrs(year: number): CalendarAttribute[] {
    ...
    const titles = [
      { kh: 'ពិធីបុណ្យចូលឆ្នាំថ្មី ប្រពៃណីជាតិ (មហាសង្រ្កាន្ត)', en: "Khmer New Year's Day (Moha Sangkranta)" },
      { kh: 'ពិធីបុណ្យចូលឆ្នាំថ្មី ប្រពៃណីជាតិ (វារៈវ័នបត)', en: "Khmer New Year's Day (Veareak Vanabat)" },
      { kh: 'ពិធីបុណ្យចូលឆ្នាំថ្មី ប្រពៃណីជាតិ (វារៈឡើងស័ក)', en: "Khmer New Year's Day (Veareak Laeung Sak)" },
    ];
    return days.map((date, i) => ({
      key: 'traditional_holidays' + moment(date).locale('en').format('YYYY-MM-DD'),
      customData: {
  ...
  prevMonth(): void {
    this.currentMonth.update((m) => { if (m === 1) { this.currentYear.update((y) => y - 1); return 12; } return m - 1; });
  }
  nextMonth(): void {
    this.currentMonth.update((m) => { if (m === 12) { this.currentYear.update((y) => y + 1); return 1; } return m + 1; });
  }
  prevYear(): void { this.currentYear.update((y) => y - 1); }
  nextYear(): void { this.currentYear.update((y) => y + 1); }
  reset(): void { this.currentMonth.set(moment().month() + 1); this.currentYear.set(moment().year()); }
}